...
def esp():
    messagebox.showwarning("Cambiar idioma", "Trabajo en progreso")

def eng():
    messagebox.showwarning("Change language", "Work in progress")
...
def black_plus_theme():
    raiz.style.theme_use("cyborg")

def about():
    messagebox.showinfo("Gracias por usar", "Hecho en Python por \nJavier Corrales\n\nContacto:\nlaboral.corrales@gmail.com")
...
def calcular():
    ...
    try:
        if variableCod_Prod.get()=="" or variableProd.get()=="" or variableEditorial.get()=="" or variablePrecioCosto.get()=="":
            messagebox.showwarning("Atención!", "No deje ningún cuadro vacio!")
...
accesoBD.add_command(label="Conectar", command=conectar_base)
...
tema.add_command(label="Pupura", command=purple_theme)
tema.add_command(label="Oscuro", command=black_theme)
tema.add_command(label="Negro", command=black_plus_theme)
...
lineademenu.add_cascade(label="Archivo BBDD", menu=accesoBD)
